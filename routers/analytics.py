...
def get_sheet_invocations(
    ...
    logs = analytics_handler.get_api_logs(account_id, sheet_api_name, start_time)
    return [ApiInvocationResponse.from_db_dict(log) for log in logs]


@router.get("/get-api-invocations-total")
def get_sheet_invocations_total(account_id: str, sheet_api_name: str):
    """
    ...
    """
    return analytics_handler.get_api_total_invocations(account_id, sheet_api_name)
