...
[Google Oauth](https://developers.google.com/identity/protocols/oauth2) is used for user authentication, integrated with Starlette sessions. User refresh tokens are encrypted using envelope encryption and AWS Key Management Service.

### Infrastructure

The app is hosted on AWS and defined by a Cloudformation Template. Front to back, the infrastructure is:

- **ECR**: Repository containing the App's Docker Image(s)
- **Lambda**: Serverless function(s) that executes containers from the Docker image
