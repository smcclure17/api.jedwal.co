...
class PostCreateRequest(BaseSchema):
    ...
    google_doc_id: str = (
        Field(..., description="The Google Doc ID to create a post from"),
    )


class PostCreate(BaseSchema):
    owner_id: str = (
        Field(..., description="The post owner (organization or account ID)"),
    )
    post_key: str = (Field(..., description="The name/ID/slug of the post"),)
    google_doc_id: str = (
        Field(..., description="The Google Doc ID to create a post from"),
...
class Post(PostBase):
    ...
    categories: list[str] | None = Field(default=None, description="Post categories (Denormalized)")
