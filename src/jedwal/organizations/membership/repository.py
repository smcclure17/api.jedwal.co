...
def delete_membership(*, table: Table, account_id: AccountId, organization_id: str) -> None:
    try:
        table.delete_item(
            Key={
...
def get_memberships_for_account(*, table: Table, account_id: AccountId) -> list[Membership]:
    response = table.query(
        IndexName="GSI1",
